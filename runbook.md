...
**Ask mode (why questions are reliable even on llama-3.3):** free-tier `llama-3.3-70b-versatile` is weak at *calling* tools, so **questions don't use the tool loop**. When your message looks like a question (not a clear edit request), the backend does the search itself, injects the results, and asks the model for a plain text answer — nothing for it to garble. Edit/modify requests ("add…", "fix…", "refactor…") still use the agentic path where the model calls tools (retry-guarded). Routing is automatic; you'll see `mode=ask` or `mode=agent` in the `[agent] chat …` log line. Force it off with `AGENT_ASK_MODE=off` (all turns agentic).
