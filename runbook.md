...
- **Changes** ("change the theme to dark", "add a footer", "fix…") → the backend reads the relevant files, the model replies with `SEARCH/REPLACE` edit blocks (plain text), and the **backend applies them** and reloads the preview. The model never calls a `write` tool, so it can't garble the call. You'll get a summary like `Applied 2 change(s): - app/page.tsx — edited`.

Routing is automatic; the `[agent] chat …` log shows `mode=ask` / `mode=edit` / `mode=agent`. If an edit reports `SEARCH text not found — skipped`, the model's quoted lines didn't match the file — rephrase or name the exact file. To use the old tool-driven path (only worth it with a strong tool-calling model), set `AGENT_EDIT_STRATEGY=agentic`.
