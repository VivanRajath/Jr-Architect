...
**Ask mode (retrieve-then-generate, toolless):** agentic retrieval needs the model to reliably *call* `search_code`/`read`, and `llama-3.3-70b-versatile` is weak at function-calling (it garbles calls → `tool call validation failed` / `Failed to call a function`). So question-type turns bypass the tool loop entirely: `resolveTurnMode()` routes a message to **ask** unless it shows clear file-modification intent (`EDIT_INTENT` regex) or the client sends an explicit `mode`. In ask mode the **backend** does the retrieval — `buildAskPrompt()` pulls salient terms from the question, runs `search_code` server-side, and injects the `file:line` hits into the prompt (the repo map is already in context via always-loaded knowledge) — then calls the model with `replaceBuiltinTools: true` + `allowedTools: []`, so the request carries **zero tools** and the model only has to write a grounded answer. This makes "ask/summarize" reliable on a weak-tool-calling model. Edits still use the agentic path (writing files needs the `write` tool), guarded by the retry loop. Both paths share `streamTurn()`. Disable ask routing with `AGENT_ASK_MODE=off`.
